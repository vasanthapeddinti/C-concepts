/* Very loose implementation of grep */
...
int strindex(char s[], char t[]) {
    ...
    for (i = 0; s[i] != '\0'; i++) {
        ...
        if (k > 0 && t[k] == '\0') {
            return i; /* return the index of the match in original string s[] */
        }
    }
    return -1; // If not present, return -1 as negative index does not exist
}